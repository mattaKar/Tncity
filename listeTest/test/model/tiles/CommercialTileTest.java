...
/**
 * list of test checking the class Commercial
 * @author guilain
 *
 */
public class CommercialTileTest {
	...
	public void Testinit() {
		CommercialTile res = new CommercialTile();
        Assert.assertEquals(CommercialTile.DEFAULT_WORKERS_CAPACITY, res.getworkersCapacity());
        res = new CommercialTile(10);
    ...
    public void testEvolve() {
        ...
        resources.increaseProductsCapacity(extraProduction);
        resources.storeProducts(extraProduction);
        resources.increaseHappiness(1000);
        ppt.setCoord(0, 0);
        ...
        assertEquals(initialValue, resources.getProductsCount());
    }
    ...
    public void testEvolveEnergieWaterRoad() {
        ...
        ppt.setCoord(0, 0);
        resources1.increaseHappiness(1000);
        resources2.increaseHappiness(1000);
        resources3.increaseHappiness(1000);
    ...
    public void testUpdate(){
        ...
        int initialValue = resources.getProductsCount();
        int consomption = ppt.getConsumption(resources);
        ppt.update(resources);
        assertEquals(initialValue - consomption, resources.getProductsCount());
    	
    }
    
    /**
     * check if the difference between a commercialTIle and a IndustrialTile give the increase of product stored
     */
    @Test
    public void testProducConsump(){
    	CommercialTile pptC = new CommercialTile();
    	IndustrialTile pptI = new IndustrialTile();
        CityResources resources = new CityResources(100, heigth, width);
        int extraProduction = 10000;
        resources.increaseEnergyProduction(extraProduction);
        resources.increaseWaterProduction(extraProduction);
        resources.increasePopulationCapacity(extraProduction);
        resources.increasePopulation(extraProduction);
        ...
        pptC.setCoord(0, 0);
        pptI.setCoord(0, 2);
        pptI.evolve(resources);
        pptC.evolve(resources);
        
        for(int i=0;i<10;i++){
        	pptI.update(resources);
            pptC.update(resources);
        }
        
        int initialValue = resources.getProductsCount();
        int consomption = pptC.getConsumption(resources);
        int production = pptI.getProduction(resources);
        pptI.update(resources);
        pptC.update(resources);
        assertEquals(initialValue + production - consomption, resources.getProductsCount());
        assertFalse(resources.getProductsCount()==initialValue);
    }
